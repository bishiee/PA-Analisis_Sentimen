    ...
    if request.method == 'POST':
        ...
        if data['type'] == 'text':
            ...
            text_preprocess = TextPreprocessing(
                text_dataframe['text'], None, PATH)
            result = text_preprocess.output(export=False)
            features_req_input = tfidf.transform(
                result['text_string_stemmed']).toarray()
...
def api_counts_words():
    if request.method == 'POST':
        ...
        count_result_Positive = count.count_words_label(
            df['text_string_stemmed'].loc[df['label'] == 'Positif'], words=10)
        count_result_Neutral = count.count_words_label(
            df['text_string_stemmed'].loc[df['label'] == 'Netral'], words=10)
        count_result_Negative = count.count_words_label(
            df['text_string_stemmed'].loc[df['label'] == 'Negatif'], words=10)
        count_json = {
...
def api_get_info():
    if request.method == 'GET':
        ...
        # Get All Stopwords
        txt_stopword = pd.read_csv(PATH['STOPWORDS'], names=[
                                   'stopwords_id'], header=None)
...
def api_file_merge():
    if request.method == 'POST' and 'file' in request.files:
        ...
        if file_upload != '' or file_upload != None:
            file_upload.save(os.path.join(
                'dataset/dataframe', file_upload.filename))
            if len(glob.glob('dataset/dataframe/*.csv')) != 0:
                ...
                combined_csv.to_csv("dataset/dataframe/df.csv",
        ...
        if data['command'] == 'reset':
            for file_name in glob.glob('dataset/dataframe/*.csv'):
                os.remove(file_name)
            copyfile('dataset/dataframe/backup/df.csv',
                     'dataset/dataframe/df.csv')
...
def api_model_train():
    if request.method == 'POST':
        ...
        full_text_pre = TextPreprocessing(
            df_csv['text'], df_csv['sentiment'], PATH)
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            text_feature, text_feature_label, random_state=1, test_size=0.2)
